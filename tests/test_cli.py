...
@pytest.fixture
def mock_go_repo(tmp_path):
    if not shutil.which("git"):
        pytest.skip("git is required for Go CLI tests")
    ...
    subprocess.run(["git", "add", "go.mod", "CHANGELOG.md"], cwd=tmp_path, check=True)
...
def test_cli_bump_go(mock_go_repo, monkeypatch):
    monkeypatch.chdir(mock_go_repo)
    with patch("rich.prompt.Confirm.ask", return_value=True):
        result = runner.invoke(app, ["bump", "minor"])
        assert result.exit_code == 0
        assert "0.1.0" in result.output
        assert "0.2.0" in result.output
        tags = subprocess.run(["git", "tag", "-l"], cwd=mock_go_repo, check=True, capture_output=True, text=True).stdout
        assert "v0.2.0" in tags
        assert "## [0.2.0]" in (mock_go_repo / "CHANGELOG.md").read_text()


def test_cli_preview_go(mock_go_repo, monkeypatch):
    monkeypatch.chdir(mock_go_repo)
    result = runner.invoke(app, ["preview"])
    assert result.exit_code == 0
    assert "Version: 0.1.0" in result.output
...
def test_cli_release_go(mock_go_repo, monkeypatch):
    monkeypatch.chdir(mock_go_repo)
    with patch("rich.prompt.Confirm.ask", return_value=True), \
